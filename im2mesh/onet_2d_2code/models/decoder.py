...
class LocalDecoder(nn.Module):
    ...
    def forward(self,p, p_project, c_global, c_local, **kwargs):
        ...
        # print('$$$$$$$$$$$$$$', c_local.shape, c_global.shape)  (32, 32, 224, 224]) torch.Size([32, 256]
        ...
        #############################################
        # c_global = self.fc_global(c_global)
        ###########################################
        c_global = torch.unsqueeze(c_global, 1)
        c_global = c_global.expand(batch_size, T, self.c_dim_global)

        # c_local = c_local / torch.norm(c_local)
        # c_global = c_global / torch.norm(c_global)
        # print('############', c_local.shape, c_global.shape)
