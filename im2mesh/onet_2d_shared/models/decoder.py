...
class LocalDecoder(nn.Module):
    ...
    def __init__(self, dim=2, c_dim_local=128, c_dim_global=128, z_resolution = 32,
                 hidden_size=256, n_blocks=5, leaky=False, sample_mode='bilinear', padding=0.1, normalize=False):
        super().__init__()
        ################################3
    ...
    def forward(self,p, p_project, c_global, c_local, **kwargs):
        ...
        if self.c_dim_local != 0:
            c_local = self.sample_plane_feature(p_project, c_local)
        ...
        p = p.float()
        net = self.fc_p(p)
        # print('*******', net.shape) #[64, 127, 256]
        # print('(((((((((((', c.shape) #[64, 127, 256]
        ...
        # add local feature as residual
        out_local = torch.cat([out.unsqueeze(-1), c_local], dim=-1)
        out_local = self.local_layer(out_local)
        out = out + out_local.squeeze(-1)
...
class LocalDecoder_CBatchNorm(nn.Module):
    ...
    def __init__(self, dim=2, c_dim_local=128, c_dim_global=128, z_resolution = 32,
                 hidden_size=256, n_blocks=5, leaky=False, legacy=False, sample_mode='bilinear', padding=0.1, normalize=False):
        super().__init__()
        ################################3
        self.fc_global = nn.Linear(256, c_dim_global)
        ####################################
    ...
    def forward(self, p, p_project, c_global, c_local, **kwargs):
        ...
        in_img = (p_project[:, :, 0] >= -1.0) & (p_project[:, :, 0] <= 1.0) & (p_project[:,:, 1] >= -1.0) & (p_project[:, :, 1] <= 1.0)
        if self.c_dim_local != 0:
            c_local = self.sample_plane_feature2(p_project, c_local)
            c_local = c_local.transpose(1, 2)
            # out of image plane is always set to 0
            c_local = in_img[:, :, None] * c_local
            # diff = (c_local - c_local2).sum(-1)
            # odd = (diff!=0)
            # coor = p_project * odd[:, :, None]
